...
function generarPDF() {
  const element = document.querySelector('#formIncidencia'); // Selecciona el contenido a convertir a PDF

  const opt = {
    margin: 1,
    filename: 'Detalle de incidencia.pdf',
...
function generarPDF() {
  const element = document.querySelector('#formIncidencia');
  const htmlContent = element.innerHTML;

  fetch('./pdf/generarPDF.php', {
    method: 'POST',
    headers: {
