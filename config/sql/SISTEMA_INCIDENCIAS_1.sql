...
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'SISTEMA_INCIDENCIAS_1')
BEGIN
    DROP DATABASE SISTEMA_INCIDENCIAS_1;
END
GO

CREATE DATABASE SISTEMA_INCIDENCIAS_1;
GO

USE SISTEMA_INCIDENCIAS_1;
GO
...
-- CREACION DE LA TABA ASIGNACION
CREATE TABLE ASIGNACION(
    ASI_numero SMALLINT IDENTITY(1,1) NOT NULL,
    ASI_fecha DATE NOT NULL,
    ASI_hora TIME NOT NULL,
    REC_numero SMALLINT NOT NULL,
    USU_codigo SMALLINT NOT NULL,
    CONSTRAINT pk_asignacion PRIMARY KEY (ASI_numero),
	CONSTRAINT fk_asignacion_recepcion FOREIGN KEY (REC_numero) 
	REFERENCES RECEPCION (REC_numero),
	CONSTRAINT fk_asignacion_usuario FOREIGN KEY (USU_codigo) 
...
CREATE TABLE CIERRE(
    ...
    CONSTRAINT fk_asignacion_cierre FOREIGN KEY (ASI_numero) 
	REFERENCES ASIGNACION (ASI_numero)
);
GO
...
CREATE TABLE AUDITORIA (
	...
	CONSTRAINT fk_auditoria_usuario FOREIGN KEY (USU_codigo) 
	REFERENCES USUARIO (USU_codigo)
);
GO
...
BEGIN
    ...
    BEGIN TRY 
        ...
        BEGIN
            -- Insertar la nueva recepción, el trigger generará el REC_numero automáticamente
            ...
            -- Obtener el REC_numero recién generado por el trigger
            DECLARE @REC_numero SMALLINT;
            SELECT @REC_numero = REC_numero
            FROM RECEPCION
            WHERE REC_fecha = @REC_fecha
              AND REC_hora = @REC_hora
              AND INC_numero = @INC_numero
              AND PRI_codigo = @PRI_codigo
              AND IMP_codigo = @IMP_codigo
            ...
            -- Registrar una asignación para la recepción
            INSERT INTO ASIGNACION (ASI_fecha, ASI_hora, REC_numero, USU_codigo)
            VALUES (@REC_fecha, @REC_hora, @REC_numero, @USU_codigo);
        END
        ELSE
        BEGIN
